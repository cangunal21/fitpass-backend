...
/**
 * TEST KULLANICILARI DOĞRULANMIŞ DOĞAR.
 *
 * E-posta doğrulama kapısı (middlewares/requireVerified.ts) rezervasyon, yorum, sosyal yazma ve
 * bekleme listesi uçlarını doğrulanmamış hesaba kapatıyor. Bu suite'teki testlerin ~100'ü
 * kullanıcıyı doğrudan Prisma ile kuruyor; hiçbiri KAPININ KENDİSİNİ test etmiyor — hepsi
 * rezervasyon/puan/sosyal mantığını test ediyor. Her birine tek tek `isEmailVerified: true`
 * eklemek hem gürültü hem de yeni yazılan testte unutulacak bir tuzak olurdu.
 *
 * Bu yüzden TEST TARAFINDAKİ istemciye tek bir uzantı takıyoruz: doğrudan yaratılan kullanıcılar
 * doğrulanmış doğar. Uzantı YALNIZ bu süreçte geçerlidir — sunucu kendi Prisma istemcisini
 * kullanır, yani /api/auth/register ile açılan hesaplar GERÇEK akıştan geçer ve doğrulanmamış
 * kalır. Kapının kendisi ayrı ve açık bir testle doğrulanıyor
 * ("Kayıt: e-posta doğrulama kodu...").
 */
const testPrisma = prisma.$extends({
  query: {
    user: {
      async create({ args, query }) {
        args.data = { isEmailVerified: true, ...args.data }
        return query(args)
      },
      async upsert({ args, query }) {
        args.create = { isEmailVerified: true, ...args.create }
        // update dalı da: testler sabit ID'lerle upsert ediyor, yani ÖNCEKİ koşudan kalan satır
        // (isEmailVerified=false) yeniden kullanılıyor. Yalnız create'i yamamak, ikinci koşuşta
        // testlerin düşmesine yol açardı — "ilk koşuda geçer, sonra kırılır" en kötü test türü.
        args.update = { isEmailVerified: true, ...args.update }
        return query(args)
      },
    },
  },
...
async function seed() {
  ...
  await testPrisma.user.upsert({ where: { id: U }, update: { preferredSports: [catName], preferredNeighborhoods: [V] }, create: { id: U, username: `smoke_${U}`, email: `smoke_${U}@x.com`, passwordHash: 'x', fullName: 'Smoke User', preferredSports: [catName], preferredNeighborhoods: [V] } })
...
async function run() {
  ...
  await check('Gizlilik: anonim yorumda reviewer + reviewerUserId sızmıyor', async () => {
    ...
    await testPrisma.user.upsert({ where: { id: Y }, update: {}, create: { id: Y, username: `anon_${Y}`, email: `anon_${Y}@x.com`, passwordHash: 'x', fullName: 'Anon User' } })
  ...
  await check('Ban: yorumlar silinir + salon puanı yeniden hesaplanır', async () => {
    ...
    await testPrisma.user.upsert({ where: { id: X }, update: { banned: false }, create: { id: X, username: `revban_${X}`, email: `revban_${X}@x.com`, passwordHash: 'x', fullName: 'RevBan' } })
  ...
  await check('Referral: davet→ücretli ders→100+100, pending→completed, 3-limit, silme-decrement', async () => {
    ...
    const reg = async (tag: string, refCode?: string) => {
      ...
      if (u) await prisma.user.update({ where: { id: u.id }, data: { isEmailVerified: true } })
  ...
  await check('Girdi cap: uzun fullName (register) + notes (booking) kırpılır', async () => {
    ...
    // Bu test GERÇEK kayıt akışını kullanıyor → kullanıcı doğrulanmamış doğuyor ve rezervasyon
    // kapısına takılır. Testin konusu girdi kırpma, doğrulama kapısı değil (onun kendi testi var).
  ...
  await check('Chat: geçmiş DB\'den okunmaz (KVKK — saklama kaldırıldı)', async () => {
    const CU = 990111
    await testPrisma.user.upsert({ where: { id: CU }, update: {}, create: { id: CU, username: `chat_${CU}`, email: `chat_${CU}@x.com`, passwordHash: 'x', fullName: 'Chat' } })
  ...
  await check('Favoriler: donmuş salon listede yok, geri aktifleşince döner', async () => {
    ...
    await testPrisma.user.upsert({ where: { id: FU }, update: {}, create: { id: FU, username: `fav_${FU}`, email: `fav_${FU}@x.com`, passwordHash: 'x', fullName: 'Fav' } })
  ...
  await check('Yorum: seans silinince yorum KALICI (ayrıştırılır), salon puanı düşmez', async () => {
    ...
    await testPrisma.user.upsert({ where: { id: RU }, update: {}, create: { id: RU, username: `rev_${RU}`, email: `rev_${RU}@x.com`, passwordHash: 'x', fullName: 'Rev' } })
  ...
  await check('Puanlama: check-in\'li + bitişten 2sa sonra salon & hoca çift puanı + display', async () => {
    ...
    await testPrisma.user.upsert({ where: { id: IU }, update: {}, create: { id: IU, username: `rate_${IU}`, email: `rate_${IU}@x.com`, passwordHash: 'x', fullName: 'Rate User' } })
  ...
  await check('Puanlama: check-in olmayan booking 403 (derse gitmeyen puanlayamaz)', async () => {
    ...
    await prisma.class_Session.upsert({ where: { id: ISS }, update: { startsAt: past3h, endsAt: past3h }, create: { id: ISS, classId: IC, startsAt: past3h, endsAt: past3h, capacity: 20, status: 'open' } })
    await testPrisma.user.upsert({ where: { id: IU }, update: {}, create: { id: IU, username: `nc_${IU}`, email: `nc_${IU}@x.com`, passwordHash: 'x', fullName: 'NoCheck' } })
  ...
  await check('Puanlama: /pending bekleyen dersi verir + job hatırlatma gönderir + puanlayınca listeden düşer', async () => {
    ...
    await testPrisma.user.upsert({ where: { id: IU }, update: {}, create: { id: IU, username: `pend_${IU}`, email: `pend_${IU}@x.com`, passwordHash: 'x', fullName: 'Pend User' } })
  ...
  await check('Eğitmen auth: davet→şifre→giriş→me(finans yok)→yanıt; salon/eğitmen token izolasyonu', async () => {
    ...
    await testPrisma.user.upsert({ where: { id: IU }, update: {}, create: { id: IU, username: `arev_${IU}`, email: `arev_${IU}@x.com`, passwordHash: 'x', fullName: 'A Rev' } })
  ...
  await check('Eğitmen portalı: profil-düzenle + ders/seans ekle + check-in (sahiplik + finans yok + onay)', async () => {
    ...
    await testPrisma.user.upsert({ where: { id: IU }, update: {}, create: { id: IU, username: `portal_${IU}`, email: `portal_${IU}@x.com`, passwordHash: 'x', fullName: 'Portal User' } })
  ...
  await check('Grup etiketleme: self-tag + duplikat temizlenir, arkadaş 1 davet bildirimi', async () => {
    ...
    await testPrisma.user.upsert({ where: { id: T }, update: { username: tName, email: `${tName}@x.com`, banned: false }, create: { id: T, username: tName, email: `${tName}@x.com`, passwordHash: 'x', fullName: 'Tagger' } })
    await testPrisma.user.upsert({ where: { id: F }, update: { username: fName, email: `${fName}@x.com`, banned: false }, create: { id: F, username: fName, email: `${fName}@x.com`, passwordHash: 'x', fullName: 'Friend' } })
  ...
  await check('Streak liderliği: no-show günü seri saymaz (check-in tutarlı)', async () => {
    ...
    await testPrisma.user.upsert({ where: { id: X }, update: { neighborhoodId: N, activityPrivacy: 'public', banned: false }, create: { id: X, username: `strk_${X}`, email: `strk_${X}@x.com`, passwordHash: 'x', fullName: 'Streak User', neighborhoodId: N, activityPrivacy: 'public' } })
  ...
  await check('Sezonluk liderlik: geçen sezon dersi sayılmaz + yanıtta sezon bilgisi var', async () => {
    ...
    await testPrisma.user.upsert({ where: { id: X }, update: { neighborhoodId: N, activityPrivacy: 'public', banned: false }, create: { id: X, username: `szn_${X}`, email: `szn_${X}@x.com`, passwordHash: 'x', fullName: 'Sezon User', neighborhoodId: N, activityPrivacy: 'public' } })
  ...
  await check('Liderlik: aktivite-gizli kullanıcı da sıralamada görünür (yeni model)', async () => {
    ...
    await testPrisma.user.upsert({ where: { id: PU }, update: { neighborhoodId: N, activityPrivacy: 'private', banned: false }, create: { id: PU, username: `lbgiz_${PU}`, email: `lbgiz_${PU}@x.com`, passwordHash: 'x', fullName: 'LbGizli', tierId: 1, neighborhoodId: N, activityPrivacy: 'private' } })
  ...
  await check('Waitlist: dolu seans → sıra → iptalde bildirim → rezervasyonda listeden çık', async () => {
    ...
    for (const uid of [UA, UB, UC]) {
      await testPrisma.user.upsert({ where: { id: uid }, update: {}, create: { id: uid, username: `wl_${uid}`, email: `wl_${uid}@x.com`, passwordHash: 'x', fullName: `WL ${uid}` } })
    }
  ...
  await check('Salon dondurma: donmuş salonun seansı liste/detay/rezervasyonda kapalı', async () => {
    ...
    await testPrisma.user.upsert({ where: { id: U2 }, update: {}, create: { id: U2, username: `lc_${U2}`, email: `lc_${U2}@x.com`, passwordHash: 'x', fullName: 'LC User' } })
  ...
  await check('Transfer: ucuz derse geçişte pointsEarned tazelenir, BAKİYE değişmez', async () => {
    ...
    await testPrisma.user.upsert({ where: { id: TU }, update: { rewardPoints: 2, tierId: 1 }, create: { id: TU, username: `trf_${TU}`, email: `trf_${TU}@x.com`, passwordHash: 'x', fullName: 'Transfer User', tierId: 1, rewardPoints: 2 } })
  ...
  await check('Salon istatistik: doluluk groupSize (koltuk) sayar, kayıt değil', async () => {
    ...
    await testPrisma.user.upsert({ where: { id: SU }, update: {}, create: { id: SU, username: `stat_${SU}`, email: `stat_${SU}@x.com`, passwordHash: 'x', fullName: 'Stat User', tierId: 1 } })
  ...
  await check('Güvenlik: public profil bookings checkInCode/finansal alan SIZDIRMAZ', async () => {
    const PU = 990281
    await testPrisma.user.upsert({ where: { id: PU }, update: { activityPrivacy: 'public', profilePrivacy: 'public' }, create: { id: PU, username: `pub_${PU}`, email: `pub_${PU}@x.com`, passwordHash: 'x', fullName: 'Pub User', tierId: 1, activityPrivacy: 'public' } })
    await prisma.booking.deleteMany({ where: { userId: PU } })
  ...
  await check('Güvenlik: createBooking geçmiş seansa izin vermez (400)', async () => {
    const BU = 990283
    await testPrisma.user.upsert({ where: { id: BU }, update: {}, create: { id: BU, username: `bkp_${BU}`, email: `bkp_${BU}@x.com`, passwordHash: 'x', fullName: 'Bk', tierId: 1 } })
  ...
  await check('Ekonomik: ders silmede puan geri-alma clamp\'li (#C)', async () => {
    ...
    await testPrisma.user.upsert({ where: { id: EU }, update: { rewardPoints: 30 }, create: { id: EU, username: `econc_${EU}`, email: `econc_${EU}@x.com`, passwordHash: 'x', fullName: 'EconC', rewardPoints: 30 } })
  ...
  await check('Ekonomik: kapalı dersin seansı booklanamaz (#E)', async () => {
    ...
    await testPrisma.user.upsert({ where: { id: EU }, update: {}, create: { id: EU, username: `econe_${EU}`, email: `econe_${EU}@x.com`, passwordHash: 'x', fullName: 'EconE' } })
  ...
  await check('Sezon şampiyonu: biten sezonda ilçe+spor ilk 3 rozet + getMe kapsam/sezon', async () => {
    ...
    const mkU = (id: number) => testPrisma.user.upsert({ where: { id }, update: { neighborhoodId: N, activityPrivacy: 'public', banned: false }, create: { id, username: `smp_${id}`, email: `smp_${id}@x.com`, passwordHash: 'x', fullName: 'Şamp', tierId: 1, neighborhoodId: N, activityPrivacy: 'public' } })
  ...
  await check('Sezon şampiyonu: berabere → iki kazanan da rank 1 (#3)', async () => {
    ...
    const mkU = (id: number) => testPrisma.user.upsert({ where: { id }, update: { neighborhoodId: NT, activityPrivacy: 'public', banned: false }, create: { id, username: `brb_${id}`, email: `brb_${id}@x.com`, passwordHash: 'x', fullName: 'Brb', tierId: 1, neighborhoodId: NT, activityPrivacy: 'public' } })
  ...
  await check('Hesap silme: tamamlanmış referral +100 geri alınır (#2 farming)', async () => {
    ...
    await testPrisma.user.upsert({ where: { id: RR }, update: { rewardPoints: 250 }, create: { id: RR, username: `rr_${RR}`, email: `rr_${RR}@x.com`, passwordHash: 'x', fullName: 'Referrer', rewardPoints: 250 } })
    await testPrisma.user.upsert({ where: { id: DD }, update: { passwordHash: bcrypt.hashSync('DelPass123', 10) }, create: { id: DD, username: `dd_${DD}`, email: `dd_${DD}@x.com`, passwordHash: bcrypt.hashSync('DelPass123', 10), fullName: 'Referred' } })
  ...
  await check('Rozet: Kurucu (ilk 500 + ilk ders) + Elçi (3 tamamlanan davet)', async () => {
    ...
    await testPrisma.user.upsert({ where: { id: KU }, update: { createdAt: new Date('2020-01-01T00:00:00Z') }, create: { id: KU, username: `ku_${KU}`, email: `ku_${KU}@x.com`, passwordHash: 'x', fullName: 'Kurucu User', tierId: 1, createdAt: new Date('2020-01-01T00:00:00Z') } })
    ...
    for (const rid of RIDS) {
      await testPrisma.user.upsert({ where: { id: rid }, update: {}, create: { id: rid, username: `kr_${rid}`, email: `kr_${rid}@x.com`, passwordHash: 'x', fullName: 'Ref', tierId: 1 } })
  ...
  await check('Takip: açık→accepted+bildirim, gizli→istek+bildirim, kabul→accepted, sayaç accepted-only', async () => {
    ...
    await testPrisma.user.upsert({ where: { id: A }, update: {}, create: { id: A, username: `fa_${A}`, email: `fa_${A}@x.com`, passwordHash: 'x', fullName: 'Follower A', tierId: 1 } })
    await testPrisma.user.upsert({ where: { id: B }, update: { profilePrivacy: 'public' }, create: { id: B, username: `fb_${B}`, email: `fb_${B}@x.com`, passwordHash: 'x', fullName: 'Target B', tierId: 1, profilePrivacy: 'public' } })
  ...
  await check('Gizli hesap: içerik sahibi+onaylı takipçiye açık, yabancıya/bekleyene gizli', async () => {
    ...
    await testPrisma.user.upsert({ where: { id: OWN }, update: { profilePrivacy: 'private', activityPrivacy: 'public', preferredSports: ['Yoga'] }, create: { id: OWN, username: `gz_${OWN}`, email: `gz_${OWN}@x.com`, passwordHash: 'x', fullName: 'Gizli User', tierId: 1, profilePrivacy: 'private', activityPrivacy: 'public', preferredSports: ['Yoga'] } })
    await testPrisma.user.upsert({ where: { id: VIEW }, update: {}, create: { id: VIEW, username: `vw_${VIEW}`, email: `vw_${VIEW}@x.com`, passwordHash: 'x', fullName: 'Viewer', tierId: 1 } })
  ...
  await check('Düzenli: GEÇMİŞ sezonda 10 ders → rozet (güncel sezon 0 olsa da)', async () => {
    ...
    await testPrisma.user.upsert({ where: { id: DU }, update: {}, create: { id: DU, username: `duz_${DU}`, email: `duz_${DU}@x.com`, passwordHash: 'x', fullName: 'Duzenli User', tierId: 1 } })
  ...
  await check('Cron hatırlatma: eşzamanlı 2 tetikte tek mail (atomik reminderSent claim)', async () => {
    ...
    await testPrisma.user.upsert({ where: { id: RS }, update: { email: `rem_${RS}@x.com` }, create: { id: RS, username: `rem_${RS}`, email: `rem_${RS}@x.com`, passwordHash: 'x', fullName: 'Reminder User', tierId: 1 } })
  ...
  await check('Drop-in check-in: etkinlikten çok önce okutulamaz (zaman penceresi)', async () => {
    ...
    await testPrisma.user.upsert({ where: { id: TU }, update: {}, create: { id: TU, username: `tz_${TU}`, email: `tz_${TU}@x.com`, passwordHash: 'x', fullName: 'Tz' } })
  ...
  await check('Test borcu: getMyBookings komisyon kırılımını GİZLER (T13)', async () => {
    const MU = 991301
    await testPrisma.user.upsert({ where: { id: MU }, update: {}, create: { id: MU, username: `mb_${MU}`, email: `mb_${MU}@x.com`, passwordHash: 'x', fullName: 'Mb' } })
  ...
  await check('Test borcu: referral kodu çağrılar arası SABİT kalır (T12 koşullu yazma)', async () => {
    const RU = 991303
    await testPrisma.user.upsert({ where: { id: RU }, update: { referralCode: null }, create: { id: RU, username: `rc_${RU}`, email: `rc_${RU}@x.com`, passwordHash: 'x', fullName: 'Rc' } })
  ...
  await check('Test borcu: transferBooking eski seansın waitlist\'ini bilgilendirir (T18)', async () => {
    ...
    await testPrisma.user.upsert({ where: { id: UA }, update: {}, create: { id: UA, username: `ta_${UA}`, email: `ta_${UA}@x.com`, passwordHash: 'x', fullName: 'Ta' } })
    await testPrisma.user.upsert({ where: { id: UB }, update: {}, create: { id: UB, username: `tb_${UB}`, email: `tb_${UB}@x.com`, passwordHash: 'x', fullName: 'Tb' } })
  ...
  await check('Test borcu: deleteDropInSlot slot + katılımcıları BİRLİKTE siler (T12 transaction)', async () => {
    ...
    await testPrisma.user.upsert({ where: { id: DU }, update: {}, create: { id: DU, username: `dd_${DU}`, email: `dd_${DU}@x.com`, passwordHash: 'x', fullName: 'Dd' } })
    const cat = await prisma.sportCategory.findFirst({ where: { name: { equals: catName, mode: 'insensitive' } }, select: { id: true } })
  ...
  await check('Gamification: puan REZERVASYONDA verilmez, check-in\'de verilir (no-show farming kapalı)', async () => {
    ...
    await testPrisma.user.upsert({ where: { id: PU }, update: { rewardPoints: 0, tierId: 2 }, create: { id: PU, username: `pt_${PU}`, email: `pt_${PU}@x.com`, passwordHash: 'x', fullName: 'Pt', rewardPoints: 0, tierId: 2 } })
  ...
  await check('Gamification: liderlik branş filtresi kanonik kategoriyi kullanır (case drift yok)', async () => {
    ...
    await testPrisma.user.upsert({ where: { id: LU }, update: {}, create: { id: LU, username: `lb_${LU}`, email: `lb_${LU}@x.com`, passwordHash: 'x', fullName: 'Lb', neighborhoodId: V } })
  ...
  await check('Bildirim: salon seansı silince pushToken\'siz kullanıcıya in-app Notification yazılır', async () => {
    ...
    await testPrisma.user.upsert({ where: { id: NU }, update: { pushToken: null }, create: { id: NU, username: `ntf_${NU}`, email: `ntf_${NU}@x.com`, passwordHash: 'x', fullName: 'Ntf', pushToken: null } })
  ...
  await check('Bildirim: seans saati değişince rezervasyon sahibine bildirim + reminderSent sıfırlanır', async () => {
    ...
    const soon = new Date(Date.now() + 3 * 86400000)
    await prisma.class_Session.upsert({ where: { id: NS }, update: { startsAt: soon }, create: { id: NS, classId: NC, startsAt: soon, endsAt: new Date(soon.getTime() + 3600000), capacity: 20, status: 'open' } })
    await testPrisma.user.upsert({ where: { id: NU }, update: {}, create: { id: NU, username: `rsc_${NU}`, email: `rsc_${NU}@x.com`, passwordHash: 'x', fullName: 'Rsc' } })
    await prisma.booking.deleteMany({ where: { userId: NU } })
    await prisma.notification.deleteMany({ where: { userId: NU } })
  ...
  await check('Girdi: geçersiz mahalle → 400 (parseIntSafe, 500 değil)', async () => {
    const PU = 991002
    await testPrisma.user.upsert({ where: { id: PU }, update: {}, create: { id: PU, username: `pf_${PU}`, email: `pf_${PU}@x.com`, passwordHash: 'x', fullName: 'Pf' } })
  ...
  await check('Girdi: gizli aktivite like → 404 (existence-oracle kapalı)', async () => {
    ...
    const XU = 991003
    await testPrisma.user.upsert({ where: { id: XU }, update: {}, create: { id: XU, username: `lk_${XU}`, email: `lk_${XU}@x.com`, passwordHash: 'x', fullName: 'Lk' } })
  ...
  await check('Hata yolu: şifre değişince TÜM refresh oturumları iptal edilir (atomik)', async () => {
    const CU = 990901
    const bcryptLib = require('bcryptjs')
    await testPrisma.user.upsert({ where: { id: CU }, update: { passwordHash: await bcryptLib.hash('EskiSifre123', 12) }, create: { id: CU, username: `cp_${CU}`, email: `cp_${CU}@x.com`, passwordHash: await bcryptLib.hash('EskiSifre123', 12), fullName: 'Cp' } })
  ...
  await check('Hata yolu: ban içerik temizliği + token iptali ATOMİK (yorum kalmaz)', async () => {
    ...
    await testPrisma.user.upsert({ where: { id: BU }, update: { banned: false }, create: { id: BU, username: `ban_${BU}`, email: `ban_${BU}@x.com`, passwordHash: 'x', fullName: 'Ban' } })
  ...
  await check('Para: önceki yılda kazanılan puan, reset sonrası iptalde CARİ yılı düşürmez', async () => {
    ...
    await testPrisma.user.upsert({ where: { id: YU }, update: { rewardPoints: 5, rewardPointsYear: thisYear }, create: { id: YU, username: `yr_${YU}`, email: `yr_${YU}@x.com`, passwordHash: 'x', fullName: 'Yr', rewardPoints: 5, rewardPointsYear: thisYear } })
  ...
  await check('Yetki: askıya alınmış salona transfer YAPILAMAZ', async () => {
    ...
    await testPrisma.user.upsert({ where: { id: TU }, update: {}, create: { id: TU, username: `tr_${TU}`, email: `tr_${TU}@x.com`, passwordHash: 'x', fullName: 'Tr' } })
    await prisma.booking.deleteMany({ where: { userId: TU } })
  ...
  await check('Yetki: gizli profilin (profilePrivacy) favori listesi yabancıya kapalı', async () => {
    const FU = 990714
    await testPrisma.user.upsert({ where: { id: FU }, update: { profilePrivacy: 'private', activityPrivacy: 'public', banned: false }, create: { id: FU, username: `fav_${FU}`, email: `fav_${FU}@x.com`, passwordHash: 'x', fullName: 'Fav', profilePrivacy: 'private' } })
  ...
  await check('Yetki: banlı kullanıcının token\'ı optionalAuth\'ta viewer sayılmaz', async () => {
    ...
    await testPrisma.user.upsert({ where: { id: OU }, update: { profilePrivacy: 'private', banned: false }, create: { id: OU, username: `own_${OU}`, email: `own_${OU}@x.com`, passwordHash: 'x', fullName: 'Own', profilePrivacy: 'private' } })
    await testPrisma.user.upsert({ where: { id: BU }, update: { banned: true }, create: { id: BU, username: `bn_${BU}`, email: `bn_${BU}@x.com`, passwordHash: 'x', fullName: 'Bn', banned: true } })
  ...
  await check('Yetki: aynı çift için İKİNCİ açık şikayet oluşturulamaz', async () => {
    ...
    await testPrisma.user.upsert({ where: { id: RU }, update: {}, create: { id: RU, username: `rp_${RU}`, email: `rp_${RU}@x.com`, passwordHash: 'x', fullName: 'Rp' } })
    await testPrisma.user.upsert({ where: { id: RT }, update: {}, create: { id: RT, username: `rt_${RT}`, email: `rt_${RT}@x.com`, passwordHash: 'x', fullName: 'Rt' } })
  ...
  await check('Yetki: getVenueBookings checkInCode ve komisyon SIZDIRMAZ', async () => {
    ...
    await testPrisma.user.upsert({ where: { id: AU }, update: {}, create: { id: AU, username: `auth_${AU}`, email: `auth_${AU}@x.com`, passwordHash: 'x', fullName: 'Auth' } })
  ...
  await check('Yetki: banUser boolean olmayan ban alanını reddeder (sessiz unban yok)', async () => {
    const BU = 990702
    await testPrisma.user.upsert({ where: { id: BU }, update: { banned: true }, create: { id: BU, username: `ban_${BU}`, email: `ban_${BU}@x.com`, passwordHash: 'x', fullName: 'Ban', banned: true } })
  ...
  await check('Yetki: askıya alınmış salonun eğitmeni check-in YAPAMAZ', async () => {
    ...
    await testPrisma.user.upsert({ where: { id: SU }, update: {}, create: { id: SU, username: `susp_${SU}`, email: `susp_${SU}@x.com`, passwordHash: 'x', fullName: 'Susp' } })
  ...
  await check('Yetki: geçmiş/kapalı seansın bekleme listesine girilemez', async () => {
    ...
    await testPrisma.user.upsert({ where: { id: WU }, update: {}, create: { id: WU, username: `wlu_${WU}`, email: `wlu_${WU}@x.com`, passwordHash: 'x', fullName: 'Wlu' } })
  ...
  await check('Eşzamanlılık: parola sıfırlama TÜM oturumları kapatır ve token tek kullanımlık', async () => {
    const PU = 990601
    const bcryptLib = require('bcryptjs')
    await testPrisma.user.upsert({ where: { id: PU }, update: {}, create: { id: PU, username: `pw_${PU}`, email: `pw_${PU}@x.com`, passwordHash: await bcryptLib.hash('EskiSifre123', 12), fullName: 'Pw' } })
  ...
  await check('Eşzamanlılık: drop-in check-in İKİ kez başarılı dönemez (atomik CAS)', async () => {
    ...
    await testPrisma.user.upsert({ where: { id: TU }, update: {}, create: { id: TU, username: `cc_${TU}`, email: `cc_${TU}@x.com`, passwordHash: 'x', fullName: 'Cc' } })
    const cat = await prisma.sportCategory.findFirst({ where: { name: { equals: catName, mode: 'insensitive' } }, select: { id: true } })
  ...
  await check('Eşzamanlılık: bekleme listesinde \'notified\' kalıcı dışlama DEĞİL (süre aşımı)', async () => {
    ...
    await testPrisma.user.upsert({ where: { id: TU }, update: {}, create: { id: TU, username: `wl_${TU}`, email: `wl_${TU}@x.com`, passwordHash: 'x', fullName: 'Wl' } })
  ...
  await check('Auth: refresh token DB\'de hash olarak saklanır (düz metin değil)', async () => {
    ...
    const uid = 990361
    await testPrisma.user.upsert({ where: { id: uid }, update: {}, create: { id: uid, username: `rt_${uid}`, email: `rt_${uid}@x.com`, passwordHash: 'x', fullName: 'RT' } })
  ...
  await check('Puan: check-in\'de GÜNCEL tier oranıyla yeniden hesaplanır (stale değil)', async () => {
    ...
    await testPrisma.user.upsert({ where: { id: uid }, update: { tierId: loT.id, rewardPoints: 0 }, create: { id: uid, username: `pr_${uid}`, email: `pr_${uid}@x.com`, passwordHash: 'x', fullName: 'PR', tierId: loT.id, rewardPoints: 0 } })
  ...
  await check('Takvim: gelecekteki check-in\'li seans takvime/seriye girmez (startsAt<now)', async () => {
    const uid = 990366
    await testPrisma.user.upsert({ where: { id: uid }, update: {}, create: { id: uid, username: `cal_${uid}`, email: `cal_${uid}@x.com`, passwordHash: 'x', fullName: 'Cal' } })
    await prisma.booking.deleteMany({ where: { userId: uid } }).catch(() => {})
  ...
  await check('Auth: çıkışta cihazın push token\'ı temizlenir (hash\'li arama)', async () => {
    ...
    const uid = 990381
    await prisma.refreshToken.deleteMany({ where: { userId: uid } }).catch(() => {})
    await testPrisma.user.upsert({ where: { id: uid }, update: { pushToken: 'ExponentPushToken[smoke-logout]' }, create: { id: uid, username: `lo_${uid}`, email: `lo_${uid}@x.com`, passwordHash: 'x', fullName: 'LO', pushToken: 'ExponentPushToken[smoke-logout]' } })
    const raw = await issueRefreshToken(uid)
  ...
  await check('Admin: rozet bağlı kategori silinemez (guard tüm FK\'leri sayar)', async () => {
    ...
    await testPrisma.user.upsert({ where: { id: uid }, update: {}, create: { id: uid, username: `kd_${uid}`, email: `kd_${uid}@x.com`, passwordHash: 'x', fullName: 'KD' } })
  ...
  await check('Coğrafya: cityId istemciden değil MAHALLEDEN türetilir (İstanbul\'a sabitlenmez)', async () => {
    ...
    await prisma.user.deleteMany({ where: { id: UID } }).catch(() => {})
    await testPrisma.user.create({ data: { id: UID, username: `geo_${UID}`, email: `geo_${UID}@x.com`, passwordHash: 'x', fullName: 'Geo' } })
  ...
  await check('Yorum: özel yanıt sahip ucunda GÖRÜNÜR, public uçta GİZLİ', async () => {
    ...
    await testPrisma.user.upsert({ where: { id: RU }, update: {}, create: { id: RU, username: `rv_${RU}`, email: `rv_${RU}@x.com`, passwordHash: 'x', fullName: 'RV' } })
  ...
  await check('Bildirim i18n: DB\'ye messageKey+messageParams yazılıyor (EN kullanıcı EN metin)', async () => {
    ...
    await prisma.notification.deleteMany({ where: { userId: { in: [a, b] } } }).catch(() => {})
    await testPrisma.user.upsert({ where: { id: a }, update: {}, create: { id: a, username: `nfa_${a}`, email: `nfa_${a}@x.com`, passwordHash: 'x', fullName: 'A' } })
    // hedef kullanıcının dili EN → bildirim İngilizce üretilmeli
    await testPrisma.user.upsert({ where: { id: b }, update: { locale: 'en' }, create: { id: b, username: `nfb_${b}`, email: `nfb_${b}@x.com`, passwordHash: 'x', fullName: 'B', locale: 'en' } })
  ...
  await check('Bildirim i18n: X-Locale başlığı User.locale\'i senkronlar', async () => {
    const uid = 990373
    await testPrisma.user.upsert({ where: { id: uid }, update: { locale: 'tr' }, create: { id: uid, username: `loc_${uid}`, email: `loc_${uid}@x.com`, passwordHash: 'x', fullName: 'Loc', locale: 'tr' } })
  ...
  await check('Bekleme listesi: pencere dolunca VE kapasite artınca sıradakine haber gider', async () => {
    ...
    const mkU = (id: number) => testPrisma.user.upsert({ where: { id }, update: { banned: false }, create: { id, username: `bek_${id}`, email: `bek_${id}@x.com`, passwordHash: 'x', fullName: 'Bekleyen' } })
  ...
  await check('Puan: kredilenmemiş check-in mutabakatla tamamlanır, çift kredi olmaz', async () => {
    ...
    await testPrisma.user.upsert({ where: { id: AU }, update: { rewardPoints: 0, tierId: 1 }, create: { id: AU, username: `mut_${AU}`, email: `mut_${AU}@x.com`, passwordHash: 'x', fullName: 'Mutabakat', tierId: 1, rewardPoints: 0 } })
  ...
  await check('Tur20 küçükler: transfer hatırlatması, check-in kodu, doluluk sayımı, e-posta tarihi', async () => {
    ...
    await testPrisma.user.upsert({ where: { id: KU }, update: {}, create: { id: KU, username: `kuc_${KU}`, email: `kuc_${KU}@x.com`, passwordHash: 'x', fullName: 'Kucukler', tierId: 1 } })
  ...
  await check('Gizlilik: askıya alınan salonun yorumları gizlenir, komisyon müşteriye dönmez', async () => {
    ...
    await testPrisma.user.upsert({ where: { id: MU }, update: {}, create: { id: MU, username: `mod_${MU}`, email: `mod_${MU}@x.com`, passwordHash: 'x', fullName: 'Moderasyon', tierId: 1 } })
  ...
  // Artık kayıt kodla tamamlanıyor ve kod girilmeden yazma uçları kapalı.
  await check('Kayıt: e-posta doğrulama kodu — kod girilmeden yazma uçları kapalı, kod açar', async () => {
    ...
    if (reg.json?.requiresEmailVerification !== true) throw new Error('kayıt yanıtı requiresEmailVerification bayrağını taşımıyor — istemci kod ekranını açamaz')

    const u = await prisma.user.findUnique({ where: { email: em }, select: { id: true, isEmailVerified: true } })
    if (!u) throw new Error('kullanıcı yazılmadı')
    if (u.isEmailVerified) throw new Error('kayıt anında DOĞRULANMIŞ işaretlenmiş — kapı anlamsız kalır')

    // 1) DOĞRULANMADAN yazma uçları KAPALI olmalı (makine-okunur kodla)
    const bk = await http('/api/bookings', { method: 'POST', token: tok, body: { sessionId: S } })
    if (bk.status !== 403 || bk.json?.code !== 'EMAIL_NOT_VERIFIED') {
      throw new Error(`doğrulanmamış hesap rezervasyon yapabildi: ${bk.status} ${bk.text.slice(0, 120)}`)
    }
    // ...ama OKUMA ve kendi profili AÇIK kalmalı (yoksa kodu ulaşmayan kullanıcı hesabında kilitlenir)
    if ((await http('/api/auth/me', { token: tok })).status !== 200) throw new Error('doğrulanmamış kullanıcı kendi profilini göremiyor — kilitlenir')

    // 2) Kod DB'de HAM saklanmamalı
    const kayit = await prisma.emailVerificationToken.findFirst({ where: { userId: u.id, used: false }, orderBy: { id: 'desc' } })
    if (!kayit?.code) throw new Error('doğrulama kodu üretilmedi')
    if (/^\d{6}$/.test(kayit.code)) throw new Error('kod DB\'de HAM saklanıyor (hash bekleniyor)')

    // 3) YANLIŞ kod reddedilmeli ve deneme sayacı işlemeli
    const yanlis = await http('/api/auth/verify-code', { method: 'POST', token: tok, body: { code: '000000' } })
    if (yanlis.status !== 400) throw new Error(`yanlış kod kabul edildi: ${yanlis.status}`)
    const sayacli = await prisma.emailVerificationToken.findUnique({ where: { id: kayit.id }, select: { attempts: true } })
    if ((sayacli?.attempts || 0) < 1) throw new Error('yanlış denemede sayaç artmadı — kaba kuvvet sınırsız kalır')

    // 4) DOĞRU kod hesabı açmalı. Ham kodu testte bilmiyoruz (yalnız e-postaya gitti) → bilinen bir
    //    kodun hash'ini yazıp onu deniyoruz; doğrulama yolunun tamamı gerçek uç üzerinden koşuyor.
    const nodeCrypto = require('crypto')
    const bilinen = '246813'
    await prisma.emailVerificationToken.update({
      where: { id: kayit.id },
      data: { code: nodeCrypto.createHash('sha256').update(bilinen).digest('hex'), attempts: 0 },
    })
    const dogru = await http('/api/auth/verify-code', { method: 'POST', token: tok, body: { code: bilinen } })
    if (dogru.status !== 200 || dogru.json?.verified !== true) throw new Error(`doğru kod kabul edilmedi: ${dogru.status} ${dogru.text.slice(0, 140)}`)

    // 5) Artık rezervasyon açılmalı (kapı gerçekten kalktı mı)
    const bk2 = await http('/api/bookings', { method: 'POST', token: tok, body: { sessionId: S } })
    if (bk2.status !== 201) throw new Error(`doğrulamadan SONRA rezervasyon hâlâ kapalı: ${bk2.status} ${bk2.text.slice(0, 140)}`)

    // 6) Kod TEK KULLANIMLIK — aynı kod tekrar iş görmemeli
    // isEmailVerified'i geri alıyoruz ki uç "zaten doğrulanmış" kısayoluna düşüp erken 200
    // dönmesin — tek-kullanımlık davranışını GERÇEKTEN sınamak istiyoruz. (Uç kullanıcıyı
    // cache'ten değil doğrudan DB'den okuyor, bu yüzden ek bir invalidate gerekmiyor.)
    await prisma.user.update({ where: { id: u.id }, data: { isEmailVerified: false } })
    const tekrar = await http('/api/auth/verify-code', { method: 'POST', token: tok, body: { code: bilinen } })
    if (tekrar.status === 200) throw new Error('kullanılmış kod ikinci kez kabul edildi')
  ...
  await check('İptal: salon erteleyince kullanıcı pencere kuralından muaf TAM iade alır', async () => {
    ...
    await testPrisma.user.upsert({ where: { id: EU }, update: {}, create: { id: EU, username: `ert_${EU}`, email: `ert_${EU}@x.com`, passwordHash: 'x', fullName: 'Erteleme' } })
  ...
  await check('Yetki: rezervasyonlu seans 12 saatten YAKINA taşınamaz (iptal hakkı korunur)', async () => {
    ...
    await testPrisma.user.upsert({ where: { id: YU }, update: {}, create: { id: YU, username: `tas_${YU}`, email: `tas_${YU}@x.com`, passwordHash: 'x', fullName: 'Tasima' } })
  ...
  await check('Hatırlatma: gönderim başarısızsa reminderSent geri alınır (tekrar denenir)', async () => {
    ...
    await testPrisma.user.upsert({ where: { id: RU1 }, update: { pushToken: 'gecersiz-token', emailReminders: false }, create: { id: RU1, username: `hat_${RU1}`, email: `hat_${RU1}@x.com`, passwordHash: 'x', fullName: 'Hat1', pushToken: 'gecersiz-token', emailReminders: false } })
    // U2: HİÇBİR kanal yok (push token yok, e-posta kapalı) → denenecek bir şey yok, bayrak KALMALI
    await testPrisma.user.upsert({ where: { id: RU2 }, update: { pushToken: null, emailReminders: false }, create: { id: RU2, username: `hat_${RU2}`, email: `hat_${RU2}@x.com`, passwordHash: 'x', fullName: 'Hat2', pushToken: null, emailReminders: false } })
  ...
  await check('Yetki: onayı KALDIRILMIŞ salon check-in YAPAMAZ', async () => {
    ...
    await testPrisma.user.upsert({ where: { id: DU }, update: {}, create: { id: DU, username: `ons_${DU}`, email: `ons_${DU}@x.com`, passwordHash: 'x', fullName: 'Onay' } })
    const kod = 'ONAYKOD1'
  ...
  await check('Drop-in: özel slot kodla görüntülenir ve kodla katılınır, kodsuz kapalı', async () => {
    ...
    await testPrisma.user.upsert({ where: { id: PU }, update: {}, create: { id: PU, username: `odu_${PU}`, email: `odu_${PU}@x.com`, passwordHash: 'x', fullName: 'OzelDavetli' } })
  ...
  await check('Puan: transfer bakiyeye DOKUNMAZ (kredilenmemiş puan düşülmez/eklenmez)', async () => {
    ...
    await testPrisma.user.upsert({ where: { id: TU }, update: { rewardPoints: 100, tierId: 1 }, create: { id: TU, username: `trf_${TU}`, email: `trf_${TU}@x.com`, passwordHash: 'x', fullName: 'Trf', tierId: 1, rewardPoints: 100 } })
    const tok = jwt.sign({ userId: TU, email: `trf_${TU}@x.com` }, JWT_SECRET, { expiresIn: '1h' })
  ...
  await check('Sezon şampiyonu: rozetle birlikte BİLDİRİM de gider ve tekrar koşuda çiftlenmez', async () => {
    ...
    await testPrisma.user.upsert({ where: { id: UU }, update: { neighborhoodId: NB, activityPrivacy: 'public', banned: false }, create: { id: UU, username: `bld_${UU}`, email: `bld_${UU}@x.com`, passwordHash: 'x', fullName: 'Bildirim', tierId: 1, neighborhoodId: NB, activityPrivacy: 'public' } })
  ...
  await check('Kapasite: public uçlar KALAN yeri döndürür (kapasiteyi değil)', async () => {
    ...
    await testPrisma.user.upsert({ where: { id: CU }, update: {}, create: { id: CU, username: `cap_${CU}`, email: `cap_${CU}@x.com`, passwordHash: 'x', fullName: 'Cap' } })
  ...
  await check('Kapasite: seans güncelleme doluluğun altına düşüremez ve geçersiz değeri reddeder', async () => {
    ...
    await testPrisma.user.upsert({ where: { id: UU2 }, update: {}, create: { id: UU2, username: `cap2_${UU2}`, email: `cap2_${UU2}@x.com`, passwordHash: 'x', fullName: 'Cap2' } })
