...
guardTestDb('fuzz') // CANLI DB koruması — dotenv export edilmiş DATABASE_URL'i EZMEZ (bkz. _guardTestDb.ts)
...
async function seed() {
  ...
  await prisma.user.upsert({ where: { id: U }, update: {}, create: { id: U, username: `fuzz_${U}`, email: `fuzz_${U}@x.com`, passwordHash: 'x', fullName: 'Fuzz User' } })
